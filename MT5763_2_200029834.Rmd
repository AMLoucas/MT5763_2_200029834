...
#### [Q1] Is there an effect of Luxuriant above and beyond the placebo
  From our Box-Plot we can conclude that Luxuriant is more efficient than Placebo and by taking the p-value under consideration we know with certainty that our two means are not equal. \
...
![QQ-Plot assumption violation](QQ-assumption/Q1-assumption.png)
  Using our randomization test and comparing the original means with the randomized means difference. We view that there is not any sample mean differences more significant than our original which proves that Luxuriant hair growth length mean is larger. Result table can be contructed by running the code, but has no rows. \
...
![Boxplot comparing all 3 drugs in terms of hair growth in mm.](BOXPLOTS/Q2-BOXPLOT.jpeg)
  Viewing the histograms of each drug and using the Confidence intervals, we conclude that indeed the Luxuriant drug is not more efficient than the already existing drugs, with most efficient drug being Baldbegone. \
...
#### [Q3] Is age relevant to any effect?
  Using the model that considers all the drugs and implements the group as a factor in the model. We can view from the histogram below that the model estimates contain the value 0. This shows that Age does not affect the drug in terms of hair growth length. \
![Histogram of the models age estimation for all drugs.](HISTOGRAMS/Q3-All.png)
  ...
  We now apply a model only concerning the drug Luxuriant. Having a simple model Hair growth = Age. Our estimation histograms contains again the value 0 which proves that Age is not a significant variable. In each simulation run we had positive and negative slope values. This shows that the age was not an influence. \
