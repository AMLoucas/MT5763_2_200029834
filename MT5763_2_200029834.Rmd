...
#### For question 1 (Is there an effect of Luxuriant above and beyond the placebo?)
 We will tidy/wrangle our data up in an appropriate format and after apply a linear regression using as predictor the a factor variable of the groups 'Luxuriant' and 'PlaceBo'. Our response variable will be hair growth length in mm. \
 For our Data Wrangling we will first need to filter out the columns we are mostly interested [Luxuriant, PlaceBo]. We will then combine those two columns to one unique column that holds the hair growth length in mm and create a new column with name GROUP which will hold the group that the specific row is representing. Once we have evolved our data in the appropriate format we will apply the linear model to compare the means. \
[ Comments are supplied in the code to explain every necessary step and the wrangling in detail.]
```
...
QUIT;
```
 Once we applied our Linear model, we have noticed our assumptions are partially violated. Because our assumptions our violated we can not trust our p-values with certainty. For this reason we will apply a randomized test of the two means which does not have to follow any distribution assumptions. \
...
RUN;
```
We can now use the compared means to obtain the proportional value we are interested in. To answer our questions. \
 **For Question 2**
 \
...
Yes.
```
knitr::include_graphics("TABLES-PLOTS/Q1-MODEL-BOXPLOT.pdf")
```
