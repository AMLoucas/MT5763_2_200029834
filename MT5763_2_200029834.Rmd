...
### [Q3] Is age relevant to any effect?
  ...
  Using the model that considers all the drugs and implements the group as a factor in the model. We can view from the histogram below that the model estimates contain the value 0. This shows that Age does not affect the drug results in terms of hair growth length. \
```{r, echo=FALSE}
# All defaults
...
```
  ...
  We now apply a model only concerning the drug Luxuriant. Having a simple model Hair growth = Age. Our estimation histograms contains again the value 0 which proves that Age does not affect the drug results. In each simulation applied, our Age estimate was changing from neagtive to positive in a random way and most of the times it was close to 0 (can be viewed in the below Histogram). This indicates that the age was not an influence. \
```{r, echo=FALSE}
# All defaults
